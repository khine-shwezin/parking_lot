...
EOTXT
  ...
  #pending "add more specs as needed"
    it "can handle park with no parameter " do
    run_command(pty, "park \n")
    expect(fetch_stdout(pty)).to end_with("Need details. Try again!\n")
	end
	
	it "can handle park no parameter" do
    run_command(pty, "leave \n")
    expect(fetch_stdout(pty)).to end_with("Need a slot number. Try again!\n")
    end
end
